...
function gtext()
{
	return Cache::remember('gtext', 3600, function () {
		...
		// সব option_name একসাথে নিন — মাত্র 1 কোয়েরি
		$optionNames = [
			'general_settings',
			'theme_logo',
			'currency',
		...
		$options = Tp_option::whereIn('option_name', $optionNames)
			->pluck('option_value', 'option_name'); // ['option_name' => 'option_value']

		// JSON decode এর জন্য হেল্পার
		...
		foreach ($paymentOptions as $gateway => $fields) {
			$p = $decode($options->get($gateway));
			foreach ($fields as $field) {
				// প্রতিটি কী-এর আগে gateway প্রিফিক্স যোগ করুন
				$keyName = $gateway === 'cash_on_delivery' ? 'cod_' . $field : ($gateway === 'bank_transfer' ? 'bank_' . $field : $field);
				$data[$keyName] = $p[$field] ?? '';
			}
		}
